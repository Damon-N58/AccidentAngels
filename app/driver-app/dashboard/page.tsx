  ...
  return (
    <div className="min-h-screen bg-[#F8F9FB]">
      ...
      <div className="px-4 pt-4 pb-24 space-y-5">
        ...
        {/* Compliance status — first thing a driver needs to know */}
        <Link href="/driver-app/compliance">
          <div className={`rounded-2xl p-4 flex items-center gap-3.5 min-h-[68px] ${
            isFullyCompliant ? 'bg-[#0F6E56] text-white' : 'bg-[#F59E0B]/12 border border-[#F59E0B]/40'
          }`}>
            {isFullyCompliant
              ? <CheckCircle2 className="w-7 h-7 shrink-0" />
              : <AlertTriangle className="w-7 h-7 text-[#c77700] shrink-0" />}
            <div className="flex-1 min-w-0">
              <p className={`font-semibold text-[15px] ${isFullyCompliant ? 'text-white' : 'text-[#0F1923]'}`}>
                {isFullyCompliant ? "You're fully compliant ✓" : `${totalDocs - approvedDocs} document${totalDocs - approvedDocs !== 1 ? 's' : ''} need attention`}
              </p>
              <p className={`text-sm mt-0.5 ${isFullyCompliant ? 'text-white/85' : 'text-[#5A6474]'}`}>
                {approvedDocs} of {totalDocs} approved · Tap to view
              </p>
            </div>
            <ChevronRight className={`w-5 h-5 shrink-0 ${isFullyCompliant ? 'text-white/80' : 'text-[#5A6474]'}`} />
          </div>
        </Link>
                      ...
                      <CardContent className="p-4 flex items-center gap-3.5 min-h-[64px]">
                        ...
                        <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${
                          ...
                          {trip.status === 'COMPLETED' ? 'Done' :
                           trip.status === 'IN_PROGRESS' ? 'Live' : 'Soon'}
                        </span>
        ...
        {(pendingContracts ?? []).length > 0 && (
          <div>
            <div className="flex items-center gap-2 mb-2.5 pt-1">
              <Bell className="w-5 h-5 text-[#c77700]" />
              <h2 className="text-base font-bold text-[#0F1923]">New requests</h2>
              <span className="ml-auto text-xs font-bold bg-[#c77700] text-white rounded-full px-2.5 py-0.5">
            ...
            <div className="space-y-2.5">
              {(pendingContracts ?? []).map((contract: any) => (
                <Link key={contract.id} href={`/driver-app/contracts/${contract.id}`}>
                  <Card className="rounded-2xl border border-[#F59E0B]/40 bg-[#F59E0B]/8 shadow-none">
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between gap-3">
                        <div className="min-w-0">
                          <p className="font-semibold text-[#0F1923] text-base truncate">{contract.child.name}</p>
                          <p className="text-sm text-[#5A6474] mt-0.5 truncate">{contract.child.schoolName}</p>
                          <p className="text-sm text-[#5A6474] mt-0.5 truncate">From {contract.parent.user.name}</p>
                        </div>
                        <div className="flex flex-col items-end gap-1.5 shrink-0">
                          {contract.monthlyAmountCents > 0 && (
                            <span className="text-base font-bold text-[var(--brand-ink)]">{formatZAR(contract.monthlyAmountCents)}/mo</span>
                          )}
                          <span className="text-sm font-semibold text-[#c77700]">Tap to accept →</span>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
        ...
        {/* Active children */}
        <div>
          <h2 className="text-base font-bold text-[#0F1923] mb-2.5 pt-1">Active children</h2>
          {children.length === 0 ? (
            <div className="bg-white rounded-2xl border border-[rgba(236,61,58,0.10)] p-6 text-center">
              <p className="text-[15px] font-medium text-[#0F1923]">No active children yet</p>
              <p className="text-sm text-[#5A6474] mt-1.5">
                A parent needs to choose you as their driver first.
              </p>
            </div>
          ) : (
            <div className="space-y-2.5">
                  ...
                  <Link key={child.id} href={`/driver-app/children/${child.id}`}>
                    <Card className="rounded-2xl border border-[rgba(236,61,58,0.10)] shadow-none">
                      <CardContent className="p-4">
                        <div className="flex items-center justify-between gap-3">
                          <div className="min-w-0">
                            <p className="font-semibold text-[#0F1923] text-base truncate">{child.name}</p>
                            <p className="text-sm text-[#5A6474] mt-0.5 truncate">{child.schoolName}</p>
                            <p className="text-sm text-[#5A6474] mt-0.5 truncate">
                              {child.parent.user.name} · {child.parent.user.phone}
                            </p>
                          </div>
                          <div className="flex flex-col items-end gap-1.5 shrink-0">
                            {child.monthlyFee && (
                              <span className="text-base font-bold text-[var(--brand-ink)]">{formatZAR(child.monthlyFee)}/mo</span>
                            )}
