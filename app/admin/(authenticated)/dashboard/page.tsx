  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Total drivers',      value: totalDrivers ?? 0,   icon: <Users className="w-5 h-5 text-[#1A3F7A]" /> },
          { label: 'Active / compliant', value: activeDrivers ?? 0,  icon: <ShieldCheck className="w-5 h-5 text-[#0F6E56]" /> },
          { label: 'Children enrolled',  value: totalChildren ?? 0,  icon: <Users className="w-5 h-5 text-[#F5A623]" /> },
          { label: 'Active contracts',   value: totalContracts ?? 0, icon: <FileText className="w-5 h-5 text-[#1A3F7A]" /> },
        ].map(({ label, value, icon }) => (
          <Card key={label} className="rounded-2xl border-[rgba(26,63,122,0.10)] shadow-none">
      ...
      {(pendingReview ?? 0) > 0 && (
        <div className="bg-[#F59E0B]/10 border border-[#F59E0B]/30 rounded-xl p-3 mb-6 flex items-center gap-3">
          ...
          <a href="/admin/drivers" className="text-sm font-semibold text-[#1A3F7A] ml-auto">Review →</a>
        </div>
      )}
              ...
              {(recentDrivers ?? []).map((d: any) => (
                <tr key={d.id} className="border-b border-[rgba(26,63,122,0.05)] hover:bg-[#F8F9FB]">
                  <td className="px-4 py-3">
                    <a href={`/admin/drivers/${d.id}`} className="font-medium text-[#1A3F7A] hover:underline">{d.user.name}</a>
