...
export function MapPicker({ initialLat, initialLng, onConfirm, onCancel }: MapPickerProps) {
  ...
  return (
    <div className="space-y-3">
      <div className="w-full h-64 rounded-xl overflow-hidden border border-[rgba(26,63,122,0.15)]">
        <Map ref={mapRef} center={[position.lng, position.lat]} zoom={15}>
          ...
          >
            <MarkerContent>
              <div className="w-8 h-8 rounded-full bg-[#1A3F7A] border-[3px] border-white shadow-lg flex items-center justify-center text-white text-sm font-bold">
                P
              </div>
            </MarkerContent>
          </MapMarker>
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          disabled={confirming}
          className="flex-1 h-11 bg-[#1A3F7A] text-white font-semibold rounded-xl text-sm hover:bg-[#1A3F7A]/90 disabled:opacity-60"
        >
