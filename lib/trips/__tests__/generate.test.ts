...
// The scenario the business cares about: one driver, 15-ish kids, multiple
// schools. These lock in that EVERY child is routed with both legs — the bug
// was the afternoon path stranding all-but-one child per shared school.
describe('trip generation — multiple children, multiple schools', () => {
  it('AFTERNOON: 3 children sharing one school all get picked up AND dropped home', () => {
    ...
    const stops = buildStopsForType(kids, 'AFTERNOON')
  ...
  it('MORNING: children across two schools are all routed, pickup before dropoff', () => {
    ...
    const stops = buildStopsForType(kids, 'MORNING')
